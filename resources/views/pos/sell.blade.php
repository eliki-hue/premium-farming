{{-- resources/views/pos/sell.blade.php --}}
@extends('layouts.app')
@section('content')
@php 
$VAT_RATE = 16; 
...
$dbProducts = collect();
try {
$dbProducts = \App\Models\PosProduct::orderBy('name')->get()->map(function($product) {
return (object)[
'id' => 'db_' . $product->id, // Unique ID prefix
'name' => $product->name,
...
@endphp
<style>
.pos-wrapper { background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); min-height: 100vh; padding: 20px; }
.pos-container { max-width: 1400px; margin: 0 auto; }
.panel { background: rgb(198, 191, 191); border-radius: 16px; padding: 25px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); border: 1px solid #606b76; margin-bottom: 25px; }
.pos-content { display: flex; gap: 25px; }
@media (max-width: 992px) { .pos-content { flex-direction: column; } }
.left-panel { flex: 1; }
.right-panel { width: 400px; }
.product-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; }
.product-btn { border: 2px solid #dee2e6; border-radius: 12px; padding: 20px; text-align: center; background: white; cursor: pointer; transition: all 0.3s; height: 140px; position: relative; overflow: hidden; }
.product-btn:hover:not(.out-stock) { border-color: #0d6efd; box-shadow: 0 12px 30px rgba(0,0,0,0.15); transform: translateY(-5px); }
...
.stock-badge { position: absolute; top: 10px; right: 10px; padding: 6px 12px; border-radius: 20px; font-size: 11px; font-weight: bold; min-width: 50px; text-align: center; }
...
color: white !important; border: none; border-radius: 16px; padding: 18px 24px;
font-weight: 700; font-size: 18px; box-shadow: 0 8px 25px rgba(40,167,69,0.4);
transition: all 0.3s ease; position: relative; overflow: hidden;
}
.btn-add:hover:not(:disabled) { 
background: linear-gradient(135deg, #218838 0%, #1ea085 100%);
transform: translateY(-4px); box-shadow: 0 15px 35px rgba(40,167,69,0.5);
}
.btn-add:disabled { background: #6c757d !important; box-shadow: none; cursor: not-allowed; transform: none; }
.btn-add.ready { animation: pulse-green 2s infinite; }
@keyframes pulse-green { 0%, 100% { box-shadow: 0 8px 25px rgba(40,167,69,0.4); } 50% { box-shadow: 0 8px 35px rgba(40,167,69,0.6); } }
.payment-tabs { display: flex; gap: 12px; margin-bottom: 20px; }
.payment-tab { flex: 1; padding: 15px; border: 2px solid #3e3f40; border-radius: 12px; background: rgb(94, 91, 91); cursor: pointer; transition: all 0.3s; font-weight: 600; text-align: center; color: white; }
.payment-tab.active { border-color: #28a745; background: #606361; transform: translateY(-2px); box-shadow: 0 5px 15px rgba(40,167,69,0.2); }
.btn-mpesa { background: linear-gradient(135deg, #ff6b35, #f7931e) !important; color: white !important; border-radius: 16px; font-weight: 700; }
.btn-cash { background: linear-gradient(135deg, #28a745, #20c997) !important; color: white !important; border-radius: 16px; font-weight: 700; }
.total-section { background: linear-gradient(135deg, #198754, #157347); color: white; text-align: center; border-radius: 20px; }
.mpesa-details { background: #656461; border: 2px solid #3f3e3b; border-radius: 12px; padding: 15px; margin: 15px 0; font-size: 14px; color: white; }
.receipt-btn { background: linear-gradient(135deg, #17a2b8, #138496) !important; color: white !important; border-radius: 16px; font-weight: 700; }
.form-control-lg { border-radius: 12px; border: 2px solid #5b5e60; font-weight: 500; padding: 12px 16px; }
.input-group-text { background: #676b6e; border-radius: 12px 0 0 12px !important; border: 2px solid #57595a; color: white; }
...
.grand-total-box .display-5 {
color: #8B4513 !important;
text-shadow: 0 3px 6px rgba(0,0,0,0.4) !important;
font-weight: 900 !important;
}
.grand-total-box .fs-6 {
...
<select name="product_id" id="productSelect" class="form-select form-control-lg" required>
...
{{ $product->name }} • KSh {{ number_format($product->selling_price) }} • {{ $product->stock }} {{ $product->unit }}s{{ $product->stock != 1 ? 's' : '' }}
</option>
@endforeach
</select>
</div>
...
<input type="text" id="stockDisplay" class="form-control form-control-lg text-center bg-light" readonly>
</div>
<div class="col-lg-2 col-md-6">
<label class="form-label fw-semibold">📦 Qty</label>
<input name="quantity" id="quantityInput" type="number" class="form-control form-control-lg text-center" value="1" min="1" max="999" required>
</div>
<div class="col-lg-2 col-md-6">
<label class="form-label fw-semibold">💰 Price</label>
<input name="price" id="priceInput" class="form-control form-control-lg bg-success text-white text-center fw-bold fs-6" readonly required>
<input type="hidden" name="name" id="productName">
<input type="hidden" name="unit" id="productUnit">
</div>
<div class="col-lg-2 col-md-6">
<label class="form-label fw-semibold">💵 Total</label>
<input id="totalDisplay" class="form-control form-control-lg bg-info text-white text-center fw-bold fs-6" readonly>
</div>
<div class="col-lg-1 col-md-12">
<label class="form-label fw-semibold">&nbsp;</label>
<button type="submit" id="addBtn" class="btn btn-add w-100 py-4 fs-5 fw-bold text-white" disabled>
...
@endphp
<div class="panel total-section text-white p-4">
...
<!-- ✅ FANCY GOLDEN TOTAL BOX -->
<div class="p-4 grand-total-box rounded-4 mb-4">
...
<div class="payment-tab active" data-payment="cash">
<i class="fas fa-money-bill-wave fs-5 me-2"></i>
<div>CASH</div>
</div>
<div class="payment-tab" data-payment="mpesa">
...
<span class="input-group-text"><i class="fas fa-money-bill-wave fs-5"></i></span>
<input type="number" name="amount_paid" class="form-control form-control-lg fw-bold text-center fs-5" 
placeholder="Cash Amount" step="50" min="{{ $grandTotal }}" required>
<span class="input-group-text">KSh</span>
</div>
</div>
<button type="submit" class="btn btn-cash w-100 py-4 fs-5 fw-bold mb-3">
...
<div class="mb-4">
<div class="input-group">
<span class="input-group-text"><i class="fas fa-phone fs-5 me-1"></i></span>
<input type="tel" name="phone" class="form-control form-control-lg fw-bold text-center fs-5" 
placeholder="07XXXXXXXX" pattern="07[0-9]{8}" required maxlength="10">
<span class="input-group-text"><i class="fas fa-mobile-alt fs-5"></i></span>
</div>
</div>
...
<button type="submit" class="btn btn-mpesa w-100 py-4 fs-5 fw-bold">
...
class="btn btn-info w-100 py-4 fs-5 fw-bold receipt-btn" target="_blank">
...
<input type="number" name="selling_price" class="form-control form-control-lg" min="0" step="10" required>
</div>
<div class="col-md-4">
<label class="form-label fw-semibold">Buying Price (KSh)</label>
<input type="number" name="buying_price" class="form-control form-control-lg" min="0" step="10">
</div>
<div class="col-md-4">
<label class="form-label fw-semibold">Stock Qty *</label>
<input type="number" name="stock" class="form-control form-control-lg" min="0" value="50" required>
</div>
<div class="col-md-6">
<label class="form-label fw-semibold">Unit</label>
<select name="unit" class="form-select form-control-lg">
...
<input type="number" name="low_stock_warning" class="form-control form-control-lg" value="5" min="1">
</div>
</div>
</div>
